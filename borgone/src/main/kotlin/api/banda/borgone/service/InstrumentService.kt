...
class InstrumentService(val instrumentRepo:InstrumentRepository) {
    ...
    fun findById(idInstrument: Long) : ResponseEntity<BasicResponse>{
        val res = instrumentRepo.findByIdAndActiveTrue(idInstrument).orElseThrow{InstrumentNotFoundException("Instrument Not found with id {} $idInstrument")}
        val list = ArrayList<InstrumentResponse>()
        list.add(InstrumentResponse( instrumentId=res.idInstrument, instrumentKey = res.instrumentKey, instrumentName = res.instrumentName))
   ...
   // fun instrumentList(page:Int,number:Int) : ResponseEntity<BasicResponse>{
        //val res = instrumentRepo.findAllByActive(true, Pageable.)
